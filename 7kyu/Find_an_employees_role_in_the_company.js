...
// You code must find the employee with the matching firstName and lastName and then return 
// the role for that employee or if no employee is not found it should return 
// "Does not work here!"
...
// string with a space between the first and last name i.e. Jane Doe or just a name.

function findEmployeesRole(name) {
  // employees array preloaded 
  const match = employees.find(emp => `${emp.firstName} ${emp.lastName}` === name);
  return match ? match.role : "Does not work here!"; // Passed employees role
}